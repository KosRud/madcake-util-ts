...
interface StrictCustomEvent<T extends string, K extends object> extends CustomEvent<K> {
  symbolSrict: T; // type only, for branding
}
...
interface StrictEventTarget<T extends EventMap> extends EventTarget {
  addEventListener<K extends NonNullable<T['Basic']>[number]>(
    type: K,
    listener: EventHandler<StrictEvent<K>> | { handleEvent: EventHandler<StrictEvent<K>> } | null,
  ...
  addEventListener<K extends keyof T['Custom']>(
    type: K,
    listener:
      | EventHandler<StrictCustomEvent<K & string, NonNullable<T['Custom']>[K]>>
      | { handleEvent: EventHandler<Event> }
      | null,
    options?: boolean | AddEventListenerOptions
  ): void;

  dispatchEvent<K extends NonNullable<T['Basic']>[number] | keyof NonNullable<T['Custom']>>(
...
export function strictEvents<T extends EventMap>() {
  return [EventTarget, Event, CustomEvent] as unknown as [
    ...
    new <K extends NonNullable<T['Basic']>[number]>(type: K) => StrictEvent<K>,
